...
const Dashboard: React.FC = () => {
  ...
  const data = filteredInvoices.map(invoice => ({
    ...
    totalValue: invoice.energyElectricityValue + invoice.energySCEEEValue + invoice.publicLightingValue,
    ...
    <Layout>
      ...
      <div className="mb-4">
        ...
        >
          ...
          {clientNumbers.map(clientNumber => (
            <option key={clientNumber} value={clientNumber}>{clientNumber}</option>
          ))}
        </select>
      </div>
      <h2 className="text-2xl mb-4">Consumo de Energia (kWh)</h2>
      <ResponsiveContainer width="100%" height={400}>
        <LineChart data={data}>
          ...
          <Tooltip />
          <Legend />
          <Line type="monotone" dataKey="energyConsumption" stroke="#8884d8" name="Consumo de Energia (kWh)" />
          <Line type="monotone" dataKey="energyCompensated" stroke="#82ca9d" name="Energia Compensada (kWh)" />
        </LineChart>
      </ResponsiveContainer>
      <h2 className="text-2xl mb-4">Valores Monetários (R$)</h2>
      <ResponsiveContainer width="100%" height={400}>
        <LineChart data={data}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="referenceMonth" />
          ...
          <Tooltip />
          <Legend />
          <Line type="monotone" dataKey="totalValue" stroke="#8884d8" name="Valor Total sem GD (R$)" />
          <Line type="monotone" dataKey="savingsGD" stroke="#82ca9d" name="Economia GD (R$)" />
        </LineChart>
      </ResponsiveContainer>
    </Layout>
  );
